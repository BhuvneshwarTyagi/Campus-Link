...
class _chat_pageState extends State<chat_page> {
  ...
  bool updateReadedCount(AsyncSnapshot<DocumentSnapshot<Map<String, dynamic>>> snapshot,int messageIndex) {
    bool out = true;
    List<dynamic> member_list=snapshot.data?.data()!["Members"];

    DateTime msg_stamp=snapshot.data!.data()!["Messages"][snapshot.data!.data()!["Messages"].length-messageIndex-1]["Stamp"].toDate();
    for(var email in member_list){
      DateTime user_stamp=snapshot.data!.data()![email["Email"].toString().split("@")[0]]["Last_Active"].toDate();
      print(user_stamp.hour);
      print(msg_stamp.hour);
      if(msg_stamp.year >= user_stamp.year && msg_stamp.month>=user_stamp.month && msg_stamp.day >= user_stamp.day && msg_stamp.hour >= user_stamp.hour && msg_stamp.minute>user_stamp.minute ) {
        print(email);
        out=false;
        break;
      }
